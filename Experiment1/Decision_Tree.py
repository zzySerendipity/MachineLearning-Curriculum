# glass-lenses.txt 决策树 Decision Tree
import operator
import random
...
# ==================== 测试部分 ====================
# 计算测试准确率
acc = testRate(lensesTree, lensesLabels, testd)
print(f"测试集样本数: {len(testd)}")
print(f"测试准确率: {acc:.4f} ({acc*100:.2f}%)")

# 获取预测结果
y_true = [sample[-1] for sample in testd]
y_pred = getPred(lensesTree, lensesLabels, testd)

# 输出详细对比
print("\n预测结果对比:")
print(f"{'真实标签':<15} {'预测标签':<15} {'结果':<10}")
print("-" * 40)
for i in range(len(testd)):
    result = "正确" if y_true[i] == y_pred[i] else "错误"
    print(f"{y_true[i]:<15} {y_pred[i]:<15} {result:<10}")
...
for cls in classes:
    ...
    cls_correct = sum(1 for i in cls_indices if y_pred[i] == y_true[i])
    cls_acc = cls_correct / len(cls_indices)
    print(f"  类别 '{cls}': {cls_acc:.4f} ({cls_correct}/{len(cls_indices)})")